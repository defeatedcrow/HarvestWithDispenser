...
public class RegisterShearsJson {
	...
	public void registerMaterial(Item item, int r) {
		...
		if (!rangeMap.containsKey(item)) {
			rangeMap.put(item, r);
			String mapName = GameData.getItemRegistry().getNameForObject(item);
			DispenserHervestDC.LOGGER.info("register item as shears: " + mapName + " : range " + r);
			...
			for (Entry<String, Object> ent : jsonMap.entrySet()) {
				if (ent != null) {
					...
					if (value instanceof Map) {
						...
						range = net.minecraft.util.MathHelper.floor_float(f);
					}
